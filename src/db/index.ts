...
`);
const maturityColumns = sqlite.prepare("PRAGMA table_info(maturity_scores)").all() as Array<{ name: string }>;
if (!maturityColumns.some((column) => column.name === "origin")) {
  sqlite.exec("ALTER TABLE maturity_scores ADD COLUMN origin text NOT NULL DEFAULT 'manual'");
}

...
export async function GET() {
  ...
  try {
    rawSqlite
      .prepare(
        "CREATE TABLE IF NOT EXISTS health_check (id integer PRIMARY KEY, checked_at integer NOT NULL)",
      )
      .run();
    rawSqlite
      ...
      .run(Date.now());
    rawSqlite
      .prepare("SELECT id FROM health_check ORDER BY id DESC LIMIT 1")
      .get();
    dbOk = true;
  } catch {}
  try {
    const text = await complete({
      system: "Reply with OK and no other text.",

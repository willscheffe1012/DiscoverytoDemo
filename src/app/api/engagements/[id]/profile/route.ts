...
export async function POST(
  ...
  try {
    ...
    const rows = db
      .select()
      .from(inputs)
      .where(eq(inputs.engagementId, engagementId))
      .orderBy(inputs.createdAt)
      .all();
    if (rows.length === 0)
      return NextResponse.json(
        { error: "Add at least one pasted input before generating a profile." },
        { status: 400 },
      );
    const material = rows
      .map(
        (row) =>
          `=== INPUT: ${row.title} (${row.kind}, ${new Date(row.createdAt).toISOString()}) ===\n${row.content}`,
      )
      .join("\n\n");
    const profile = await completeJson({
      system: companyProfilePrompt,
    ...
    const latest = db
      ...
      .where(
        and(
          ...
          eq(artifacts.kind, "company_profile"),
        ),
      )
    ...
    const inserted = db
      ...
      .values({
        ...
        contentJson: JSON.stringify(profile),
